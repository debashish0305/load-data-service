...
public class BatchJobRunner {
	...
	public void runJob(CsvInputFileType jobType, String inputFolder, String fileName)
			throws JobInstanceAlreadyCompleteException, JobExecutionAlreadyRunningException,
			JobParametersInvalidException, JobRestartException {
		Step step = new StepBuilder("step-" + jobType.name(), jobRepository).<User, User>chunk(100, txManager)
				.reader(UserFileReaderFactory.create(inputFolder, fileName, USER_COLUMNS))
				.processor(new UserProcessor()).writer(new UserWriter()).build();

		Job job = new JobBuilder("job-" + jobType.name(), jobRepository).start(step).build();

		JobParameters params = new JobParametersBuilder().addString("jobType", jobType.name())
